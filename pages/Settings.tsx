        ...
        <SoftCard className="flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            {[
              ...
              { label: 'Username', value: `@${user?.username || 'não definido'}` },
              { label: 'Telefone Principal', value: user?.phoneNumber || 'Não definido' },
              { label: 'Telefone Alternativo', value: user?.secondaryPhoneNumber || 'Não definido' },
              { label: 'Email Associado', value: user?.associatedEmail || user?.email || 'Não definido' },
              { label: 'Função', value: user?.role?.replace(/_/g, ' '), blue: true },
              { label: 'Data de Criação', value: user?.createdAt || 'Não disponível' },
              { label: 'Último Login', value: user?.lastLogin || 'Agora' },
              { label: 'Status', value: user?.isBanned ? 'Inativo (Banido)' : 'Ativo', green: !user?.isBanned, red: user?.isBanned },
            ].map(row => (
              <div key={row.label} className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                ...
                <span className={`text-sm font-bold ${
                  row.blue ? 'text-[#003366] dark:text-blue-400 uppercase' :
                  row.green ? 'text-green-500' :
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    {biometricEnabled
                      ? '✓ Activa — podes entrar com impressão digital ou Face ID'
                      : 'Inactiva — activa para entrar sem digitar o PIN'}
                  </p>
                </div>
                <button
                  onClick={biometricEnabled ? handleDeactivateBiometrics : handleActivateBiometrics}
                  className={`px-4 py-2 rounded-xl font-bold text-sm transition-all active:scale-95 ${
                    biometricEnabled
                      ...
                      : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-[#003366] hover:text-white'
                  }`}
                >
              ...
              {biometricEnabled && (
                <p className="text-[10px] text-slate-400 italic mt-2">
                  Clica no botão verde para desactivar a biometria.
                </p>
              )}
            </div>
          </div>
        ...
        <SoftCard>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
              ...
              >
                <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${uiEffectsEnabled ? 'left-6' : 'left-1'}`} />
              </button>
            </div>
        ...
        <SoftCard>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={row.label} className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-2 text-slate-400">{row.icon}
                  <span className="text-xs text-slate-500 dark:text-slate-400">{row.label}</span>
                </div>
                <span className={`text-xs font-bold ${
                  row.blue ? 'text-blue-600' :
                  row.colored ? (isOnline ? 'text-emerald-600' : 'text-red-600') :
                  'text-slate-800 dark:text-white'
                }`}>{row.value}</span>
              </div>
            ))}
              ...
              <div className="p-8">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" value={accountForm.username} onChange={e => setAccountForm({ ...accountForm, username: e.target.value })}
                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-10 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Telefone Principal</label>
                      <input type="tel" value={accountForm.phoneNumber} onChange={e => setAccountForm({ ...accountForm, phoneNumber: e.target.value })}
                        placeholder="9xx xxx xxx"
                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Telefone Alternativo</label>
                      <input type="tel" value={accountForm.secondaryPhoneNumber} onChange={e => setAccountForm({ ...accountForm, secondaryPhoneNumber: e.target.value })}
                  ...
                  <div>
                    ...
                    <input type="email" value={accountForm.associatedEmail} onChange={e => setAccountForm({ ...accountForm, associatedEmail: e.target.value })}
                ...
                <div className="grid grid-cols-2 gap-3 mt-8">
                  <button onClick={() => setIsEditingAccount(false)}
                    className="py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl font-bold">
                    Cancelar
                  </button>
                  <button onClick={handleUpdateAccount}
                    className="py-3 bg-[#003366] text-white rounded-xl font-bold shadow-lg flex items-center justify-center gap-2">
