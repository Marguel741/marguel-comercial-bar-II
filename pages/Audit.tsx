  ...
  if (!canView) {
    return (
      <div className="flex flex-col items-center justify-center h-[80vh] text-center px-4">
        ...
        <p className="text-slate-500 dark:text-slate-400">Não tem permissão para visualizar os registos de auditoria.</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pl-8">
        ...
        {canControl && (
          <div className="flex items-center gap-3 bg-white dark:bg-slate-800 p-2 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            >
              <Power size={18} />
              {isAuditEnabled ? 'Logs Activos' : 'Logs Inactivos'}
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold">{isAuditEnabled ? 'A monitorizar' : 'Pausado'}</span>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between mb-2">
          ...
          <div className="font-bold">{auditMode === 'IMMUTABLE' ? 'Modo Protegido Activo' : 'Modo de Teste (Mutável)'}</div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="relative">
            ...
            <select value={userFilter} onChange={(e) => setUserFilter(e.target.value)}
              ...
              {users.filter(u => u !== 'all').map(u => <option key={u} value={u}>{u}</option>)}
            </select>
          </div>
          <div className="relative">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="p-4 font-bold text-sm uppercase tracking-wider text-slate-500">Acção</th>
                <th className="p-4 font-bold text-sm uppercase tracking-wider text-slate-500">Utilizador</th>
                ...
                {auditMode === 'MUTABLE' && <th className="p-4 font-bold text-sm uppercase tracking-wider text-slate-500 text-right">Acções</th>}
              </tr>
            </thead>
                    ...
                    {auditMode === 'MUTABLE' && (
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => updateLog(log.id, { description: log.description + ' (Editado)' }, user)}
                          ...
                          <button onClick={() => deleteLog(log.id, user)}
