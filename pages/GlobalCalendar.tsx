...
const GlobalCalendar: React.FC = () => {
  ...
  const calendarDays = useMemo(() => {
    return Array.from({ length: daysInMonth }).map((_, i) => {
      ...
      return { day, dateStr, report, isLocked, isToday };
    });
  }, [year, month, daysInMonth, salesMap, lockedDays, isDayLocked]);
  ...
  const dayData = useMemo(() => {
    ...
    const dayInventoryLogs = inventoryMap.get(cleanSelected) || null;
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 animate-fade-in pb-24 relative">
       ...
       <header className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
          <div className="ml-20">
             <h1 className="text-3xl font-bold text-[#003366] dark:text-white">Calendário Marguel</h1>
          ...
          <div className="lg:col-span-3 bg-white dark:bg-slate-800 rounded-[32px] p-6 shadow-xl border border-slate-100 dark:border-slate-700">
             ...
             <div className="grid grid-cols-7 gap-2 md:gap-3">
                ...
                {calendarDays.map(({ day, dateStr, report, isLocked, isToday }) => (
                   <button 
                     key={day} 
                     onClick={() => { triggerHaptic('selection'); setSelectedDayDetail(dateStr); setActiveTab('overview'); }}
                     className={`aspect-square rounded-2xl p-2 flex flex-col justify-between relative border transition-all text-left
                        ${report ? 'bg-blue-50/50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-800' : 'bg-slate-50 dark:bg-slate-700/20 border-transparent'}
                   ...
                   >
                      <div className="flex justify-between items-start w-full">
                         ...
                         {isLocked ? <Lock size={10} className="text-red-500" /> : report ? <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.5)]"></div> : null}
                      </div>
                      {report && (
                         <div className="overflow-hidden">
                            {report.status === ClosureStatus.FECHO_CONFIRMADO ? (
                               <p className="text-[9px] font-black text-[#003366] dark:text-blue-400 truncate">{roundKz((report.totalLifted || 0) / 1000)}k</p>
                            ) : (
                               <p className="text-[7px] font-bold text-amber-600 dark:text-amber-400 leading-tight">Fecho Parcial Registado</p>
                            )}
                         </div>
                      )}
                   </button>
             ...
             <SoftCard className="bg-[#003366] text-white border-none shadow-blue-900/20">
                ...
                <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-end">
                   ...
                   <div className="text-right">
                       ...
                       <p className="font-bold text-sm">{formatKz(monthStats.count > 0 ? roundKz(monthStats.total / monthStats.count) : 0)}</p>
                   </div>
                </div>
             </SoftCard>
               ...
               <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden border border-white/20" onClick={e => e.stopPropagation()}>
                   <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                       <div className="flex items-center gap-3">
                           <div className="w-12 h-12 rounded-2xl bg-blue-100 dark:bg-blue-900/30 text-blue-600 flex items-center justify-center"><Printer size={24} /></div>
                           <div>
                   ...
                   <div className="p-8 space-y-6">
                       <div className="space-y-2">
                           ...
                           <div className="grid grid-cols-3 gap-2">
                               {[{id:'day',label:'Dia'},{id:'week',label:'Semana'},{id:'month',label:'Mês'},{id:'quarter',label:'Trimestre'},{id:'year',label:'Ano'}].map(p => (
                                   <button key={p.id} onClick={() => setPrintPeriod(p.id as any)}
                                       className={`py-3 rounded-2xl text-[10px] font-black uppercase transition-all border ${printPeriod === p.id ? 'bg-[#003366] text-white border-[#003366] shadow-lg shadow-blue-900/20' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 border-transparent hover:border-slate-200'}`}>
                                       {p.label}
                                   </button>
                               ))}
                           </div>
                       ...
                       <div className="pt-4">
                           <button onClick={() => { triggerHaptic('success'); handlePrint(); }}
            ...
            <div className="bg-white dark:bg-slate-900 w-full max-w-5xl h-[85vh] rounded-[40px] shadow-2xl flex flex-col overflow-hidden border border-white/20" onClick={e => e.stopPropagation()}>
               ...
               <div className="p-6 md:p-8 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        ...
                        <div className="mt-1">
                            {isDayLocked(selectedDayDetail) && <span className="text-red-500 font-bold">Dia Bloqueado</span>}
               ...
               <div className="flex px-8 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700 overflow-x-auto no-scrollbar">
                  {[
                     ...
                     <button key={tab.id} onClick={() => { triggerHaptic('selection'); setActiveTab(tab.id as any); }}
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-slide-up">
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-[32px] border border-slate-100 dark:border-slate-700 space-y-4">
                                ...
                                <div className="pt-4 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
                                    ...
                                    <span className={`text-xl font-black ${dayData.isConfirmed ? 'text-[#003366] dark:text-white' : 'text-slate-400 italic'}`}>
                                        {dayData.isConfirmed ? formatKz(dayData.report?.totalLifted || (dayData.report as any)?.totals?.lifted || 0) : 'N/A'}
                                    </span>
                                </div>
                                    ...
                                    {dayData.inventory && dayData.inventory.length > 0 ? (
                                        <div className="space-y-2">
                                            <div className="flex justify-between items-center text-sm">
                                                ...
                                                <span className="font-bold text-green-600">Sim ({dayData.inventory.length} registo{dayData.inventory.length > 1 ? 's' : ''})</span>
                                            </div>
                                            <div className="flex justify-between items-center text-sm">
                                                <span className="text-slate-500 font-medium">Último Responsável</span>
                                                <span className="font-bold text-[#003366] dark:text-white">{dayData.inventory[0].performedBy}</span>
                                            </div>
                                            <div className="flex justify-between items-center text-sm">
                                                <span className="text-slate-500 font-medium">Última Hora</span>
                                                <span className="font-bold text-[#003366] dark:text-white">
                                                    {(() => {
                                                      const ts = dayData.inventory[0].timestamp;
                                                      const d = new Date(ts);
                        ...
                        <div className="space-y-6">
                           ...
                           <div className="space-y-3">
                              {(() => {
                                const discVal = (dayData.report as any)?.totals?.discrepancy ?? dayData.report?.discrepancy ?? 0;
                                ...
                                return (
                                  <div className={`p-4 rounded-2xl border flex items-center gap-3 ${isShortage ? 'bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-800/30 text-red-800 dark:text-red-300' : 'bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800/30 text-green-800 dark:text-green-300'}`}>
                                    ...
                                    <div>
                                      <p className="text-xs font-black uppercase">{isShortage ? '⚠️ Dinheiro em falta' : '✅ Dinheiro a mais'}</p>
                                      <p className="text-sm font-bold">Divergência de {formatKz(Math.abs(discVal))} detectada no fecho.</p>
                                    </div>
                                  </div>
                                );
                              })()}
                              {dayData.inventory?.some((l: any) => l.status === 'DIVERGENTE') && (
                                 <div className="p-4 bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300 rounded-2xl border border-red-100 dark:border-red-800/30 flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold">Divergência detectada em equipamentos.</p>
                                    </div>
                                 </div>
                              )}
                              {!dayData.report && (
                           ...
                           <>
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                                 <div className="bg-[#003366] text-white p-5 rounded-3xl col-span-1 sm:col-span-1">
                                   ...
                                   <p className="text-xl font-black">{formatKz((dayData.report as any)?.totals?.expected || (dayData.report as any)?.totals?.soldStock || dayData.report?.totalExpected || 0)}</p>
                                 </div>
                                 <div className="bg-green-50 dark:bg-green-900/10 p-5 rounded-3xl">
                                   ...
                                   <p className="text-xl font-black text-green-700">{formatKz(dayData.report?.totalLifted || (dayData.report as any)?.totals?.lifted || 0)}</p>
                                 </div>
                                 <div className="bg-red-50 dark:bg-red-900/10 p-5 rounded-3xl">
                                   ...
                                   <p className="text-xl font-black text-red-600">{formatKz(dayData.report?.lunchExpense || (dayData.report as any)?.financials?.lunch || 0)}</p>
                                 </div>
                              </div>
                                    ...
                                    <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                                       {(dayData.report.itemsSummary || (dayData.report as any).itemsSnapshot || []).map((item: any, idx: number) => (
                                          <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800 transition-colors">
                                             ...
                                             <td className="px-6 py-4 text-right font-bold text-slate-900 dark:text-white">{formatKz(item.revenue ?? item.total ?? 0)}</td>
                                          </tr>
                                       ))}
                                    </tbody>
                                 </table>
                                      ...
                                      {dayData.expenses.map(ex => (
                                          <div key={ex.id} className="p-5 bg-red-50/50 dark:bg-red-900/10 rounded-2xl border border-red-50 dark:border-red-900/20">
                                              ...
                                              <div className="flex items-center justify-between pt-3 border-t border-red-100 dark:border-red-900/30">
                                                  ...
                                                  <div className="text-[10px] text-slate-400 font-bold uppercase">Resp: {ex.user}</div>
                                              </div>
                                          </div>
                                      ))}
                                  </div>
                                      ...
                                      {dayData.purchases.map(purchase => (
                                          <div key={purchase.id} className="p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-[24px] border border-blue-50 dark:border-blue-900/20">
                                              ...
                                              <div className="flex items-center justify-between pt-4 border-t border-blue-100 dark:border-blue-900/30">
                                                  ...
                                                  <div className="text-[10px] text-slate-400 font-bold uppercase">Resp: {purchase.completedBy}</div>
                                              </div>
                                          </div>
                                      ))}
                                  </div>
