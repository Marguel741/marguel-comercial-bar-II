...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    const ok = usePin ? await loginByPin(pin) : await login(email, password);
    if (ok) navigate('/');
    else setError(usePin
  ...
  const handleBiometric = async () => {
    try {
      ...
      if (credential) {
        ...
        if (saved) {
          const { email: e, pin: p } = JSON.parse(saved);
          const ok = p ? await loginByPin(p) : await login(e, '');
          if (ok) navigate('/');
          else setError('Falha na autenticação biométrica. Entre com as suas credenciais.');
        } else {
          setError('Nenhuma conta registada para biometria. Entre com email e senha primeiro, depois active a biometria nas Definições.');
        }
      }
    } catch {
      setError('Biometria cancelada ou não disponível neste dispositivo.');
    }
  };
        ...
        <div className="flex flex-col items-center animate-fade-in">
          <div className="mb-5 flex flex-col items-center">
            <div className="relative flex items-center justify-center">
              <span className="font-sans font-black text-5xl tracking-tighter text-[#E3007E] relative z-10"
                style={{ filter: 'drop-shadow(0 0 20px rgba(227, 0, 126, 0.55))' }}>MG</span>
          ...
          <p className="text-white/60 text-sm mt-1 text-center">Marguel Mobile Sistema de Gestão Interna</p>
        </div>
      </div>
        ...
        <div className="soft-ui p-8 dark:bg-slate-800 dark:border-slate-700 max-w-md mx-auto">
          ...
          <div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-2xl mb-7">
            {[
              ...
              <button key={String(tab.val)} type="button"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                      placeholder="seu.email@marguel.com" autoComplete="email"
