...
const AccountStatus: React.FC = () => {
  ...
  const [newCard, setNewCard] = useState<Omit<Card, 'id' | 'balance' | 'validity'>>({
    name: '', holder: '', color: 'bg-gradient-to-br from-blue-600 to-indigo-700', type: 'Corrente'
  });
  ...
  const handleCreateCard = (e: React.FormEvent) => {
    ...
    addCard({ ...newCard, balance, validity: '12/30' });
    triggerHaptic('success');
  ...
  return (
    ...
    <div className="p-4 md:p-8 space-y-8 animate-fade-in relative pb-24">
        ...
        <div className="fixed top-10 left-1/2 -translate-x-1/2 z-[100] animate-slide-up">
          <div className="bg-white dark:bg-slate-800 text-[#003366] dark:text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border border-slate-100 dark:border-slate-700">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Operação confirmada.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {showCreateCardModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 w-full max-w-md shadow-2xl relative">
            <button onClick={() => setShowCreateCardModal(false)} className="absolute top-4 right-4 p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full text-slate-400">
              <X size={24} />
            </button>
            ...
            <form onSubmit={handleCreateCard} className="space-y-5">
              <div>
                ...
                <input type="text" required value={newCard.name} onChange={(e) => setNewCard({...newCard, name: e.target.value})}
              ...
              <div>
                ...
                <input type="text" required value={newCard.holder} onChange={(e) => setNewCard({...newCard, holder: e.target.value})}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={newCard.type} onChange={(e) => setNewCard({...newCard, type: e.target.value as any})}
                    className="w-full p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border-none soft-ui-inset font-medium dark:text-white focus:ring-2 focus:ring-[#003366] outline-none">
                ...
                <div>
                  ...
                  <input type="text" inputMode="numeric" value={initialBalance} onChange={(e) => handleAmountChange(e, setInitialBalance)}
                    placeholder="0"
                    className="w-full p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border-none soft-ui-inset font-medium dark:text-white focus:ring-2 focus:ring-[#003366] outline-none" />
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {cardColors.map((color) => (
                    <button key={color.class} type="button" onClick={() => setNewCard({...newCard, color: color.class})}
              ...
              <button type="submit" className="w-full py-4 bg-[#003366] text-white font-black rounded-xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all mt-4">
                Confirmar Criação
              </button>
            </form>
          </div>
      ...
      {showCashTPAModal.isOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 w-full max-w-sm shadow-2xl relative">
            <button onClick={() => setShowCashTPAModal({ isOpen: false, type: null })} className="absolute top-4 right-4 p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full text-slate-400">
              <X size={24} />
            </button>
            ...
            <form onSubmit={handleCashTPADebit} className="space-y-5">
              <div>
                ...
                <input type="text" inputMode="numeric" autoFocus required value={cashTPAAmount} onChange={(e) => handleAmountChange(e, setCashTPAAmount)}
                  placeholder="Ex: 10 000"
              ...
              <div>
                ...
                <textarea required value={cashTPANote} onChange={(e) => setCashTPANote(e.target.value)}
                  placeholder="Ex: Fecho de TPA, Depósito em Cartão, Pagamento..."
                  className="w-full p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border-none soft-ui-inset text-sm font-medium dark:text-white resize-none" rows={3} />
              </div>
              <button type="submit" className="w-full py-4 bg-red-500 text-white font-black rounded-xl shadow-xl hover:bg-red-600 active:scale-95 transition-all mt-2">
                Confirmar Débito
              </button>
            </form>
          </div>
      ...
      {showTransModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 w-full max-w-sm shadow-2xl relative">
            <button onClick={() => setShowTransModal(false)} className="absolute top-4 right-4 p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full text-slate-400">
              <X size={20} />
            </button>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">Preencha o valor e a nota da operação.</p>
            <form onSubmit={handleTransaction} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase">Valor (Kz)</label>
                <input type="text" inputMode="numeric" autoFocus value={amount} onChange={(e) => handleAmountChange(e, setAmount)}
              ...
              <div>
                ...
                <textarea value={note} onChange={(e) => setNote(e.target.value)}
                  placeholder={transType === 'deposit' ? "Ex: Lucro Mensal, Venda Activo..." : "Ex: Pagamento Fornecedor, Manutenção..."}
                  className="w-full p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border-none soft-ui-inset text-sm font-medium mt-1 dark:text-white resize-none" rows={2} />
              </div>
              <button type="submit" className={`w-full py-4 text-white font-bold rounded-xl shadow-lg mt-2 active:scale-95 transition-all ${
                transType === 'deposit' 
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden relative flex flex-col max-h-[90vh]">
            <div className={`p-6 text-white flex justify-between items-center ${selectedTransaction.type === 'entrada' ? 'bg-green-600' : selectedTransaction.isTransfer ? 'bg-blue-600' : 'bg-red-600'}`}>
              ...
              <button onClick={() => setSelectedTransaction(null)} className="p-2 bg-white/20 hover:bg-white/40 rounded-full transition-colors">
                <X size={24} />
              </button>
            </div>
              ...
              <div className="flex justify-between items-end border-b border-slate-100 dark:border-slate-700 pb-4">
                ...
                <div className="text-right">
                  ...
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                    ...
                    {selectedTransaction.status || 'ACTIVO'}
                  </span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={label} className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-2xl border border-slate-100 dark:border-slate-700">
                    <p className="text-[10px] text-slate-400 font-bold uppercase mb-1 flex items-center gap-1">
                      <Icon size={10} /> {label}
                    </p>
                    <p className="font-bold text-slate-700 dark:text-slate-200 text-sm uppercase">{value}</p>
                  </div>
                ))}
              </div>
                  ...
                  <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-2xl border border-red-100 dark:border-red-800">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-bold text-slate-700 dark:text-slate-200">{expenses.find(e => e.id === selectedTransaction.referenceId)?.category || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Origem</p>
                        <p className="font-bold text-slate-700 dark:text-slate-200">{expenses.find(e => e.id === selectedTransaction.referenceId)?.user || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex justify-end">
              <button onClick={() => setSelectedTransaction(null)} className="px-8 py-3 bg-[#003366] text-white font-bold rounded-2xl shadow-lg hover:scale-105 transition-all active:scale-95">
                Fechar Detalhes
              </button>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden relative border border-slate-100 dark:border-slate-700">
            <div className="p-8 text-center">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">
                Tem a certeza que deseja eliminar o cartão <span className="font-bold text-slate-800 dark:text-slate-200">"{cards.find(c => c.id === showDeleteConfirm.cardId)?.name}"</span>?
                Esta acção não pode ser desfeita, mas o histórico de transacções será preservado.
              </p>
              <div className="flex flex-col gap-3">
                ...
                <button onClick={() => setShowDeleteConfirm({ isOpen: false, cardId: null })} disabled={isDeleting}
      ...
      <style>{`
        @keyframes shine { 100% { transform: skewX(-12deg) translateX(200%); } }
        .group:hover .animate-shine { animation: shine 1.5s infinite linear; }
