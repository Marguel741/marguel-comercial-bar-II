...
const UserManagement: React.FC = () => {
  ...
  const permissionSections: Record<string, { key: string; label: string; note?: string; color?: string }[]> = {
    ...
    '12. STOCK DA RESERVA': [
      ...
      { key: 'reserve_transfer', label: 'Transferir Reserva para o Bar', note: 'Move produtos da Reserva para o Bar para reposição de stock.' },
                    ...
                    {perms.map((p) => (
                      <div key={p.key} className="flex items-center justify-between p-2 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl transition-colors">
                        <div>
                          ...
                          {p.note && (
                            <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 leading-tight max-w-[200px]">{p.note}</p>
                          )}
                        </div>
                        <button 
                          onClick={() => handleTempPermissionToggle(p.key as keyof UserPermissions)}
                          className={`w-10 h-5 rounded-full p-1 transition-all flex items-center ${tempPermissions[p.key as keyof UserPermissions] ? 'bg-green-500 justify-end' : 'bg-slate-300 dark:bg-slate-700 justify-start'}`}
                        >
