...
const Dashboard: React.FC = () => {
  ...
  const alerts = useMemo(() => {
    ...
    const firestoreAlerts = notifications
      ...
      .map(n => ({
        ...n,
        isCustom: true,
        icon: n.icon || 'AlertTriangle',
        color: n.color || (n.type === 'CRITICO' ? 'red' : n.type === 'SUAVE' ? 'amber' : n.type === 'SUCCESS' ? 'green' : 'blue'),
      }));
    list.push(...firestoreAlerts);
  ...
  const handleResolveAlert = (id: string, note?: string) => {
    resolveNotification(id, user?.name || 'Admin', note);
      ...
      <div className="px-6 pt-6">
        ...
        <div className={`bg-white dark:bg-[#0a192f] rounded-[2rem] p-6 shadow-sm mb-8 transition-all duration-300 ${isChartFullScreen ? 'fixed inset-0 z-50 m-0 rounded-none overflow-y-auto' : ''}`}>
          ...
          <div className="flex gap-2 mb-6 overflow-x-auto pb-1 scrollbar-hide">
            {(['week', 'month', 'quarter', 'year'] as const).map((range, idx) => {
              const labels = ['Semana', 'Mês', 'Trim.', 'Ano'];
              return (
                <button
                  key={range}
                  onClick={() => { setTimeRange(range); setTimeOffset(0); }}
                  className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${timeRange === range ? 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-white' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                >
                  {labels[idx]}
                </button>
              );
          ...
          <div className={`${isChartFullScreen ? 'h-[60vh]' : 'h-64'} w-full transition-all duration-300`}>
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Area type="monotone" dataKey="sales" name="Vendas" stroke="#003366" strokeWidth={3} fillOpacity={1} fill="url(#colorSales)" activeDot={{ r: 6, onClick: (_, event) => event.stopPropagation() }} />
                <Area type="monotone" dataKey="expenses" name="Despesas" stroke="#EF4444" strokeWidth={3} fillOpacity={1} fill="url(#colorExpenses)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-[#0a192f] p-4 rounded-xl shadow-sm">
                  ...
                  {selectedChartDay.details?.report ? (
                    ...
                    <p className="text-sm text-slate-400 italic">Sem fecho registado.</p>
                  )}
                </div>
                <div className="bg-white dark:bg-[#0a192f] p-4 rounded-xl shadow-sm">
        ...
        <div className="bg-white dark:bg-[#0a192f] rounded-[2rem] p-6 shadow-sm mb-8">
          ...
          <div className="space-y-6">
            {topProducts.length > 0 ? (
              ...
              <p className="text-slate-400 text-sm italic">Nenhuma venda registada ainda.</p>
            )}
          </div>
        </div>
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-lg text-slate-800 dark:text-white">Alertas Activos</h3>
            {isAdmin && (
          ...
          {showCreateAlert && (
            <div className="mb-4 p-4 bg-white dark:bg-[#0a192f] rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
              ...
              <div className="flex gap-2 mb-2 flex-wrap">
                {(['CRITICO', 'SUAVE', 'INFO', 'SUCCESS'] as const).map(type => (
                  <button
                    key={type}
          ...
          <div className="space-y-3">
            {alerts.length === 0 && <p className="text-slate-400 italic text-sm">Nenhum alerta activo.</p>}
            {alerts.map((alert) => {
                    ...
                    <div className="flex items-center gap-1">
                      {alert.isCustom && isAdmin && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setResolveModal({ id: alert.id, title: alert.title });
                          }}
          ...
          {/* Secção Resolvidos */}
          {isAdmin && notifications.filter(n => n.resolved === true).length > 0 && (
            <div className="mt-4">
              <button
                onClick={() => setShowResolved(prev => !prev)}
                className="flex items-center gap-2 text-sm text-slate-400 font-bold mb-2"
              >
                {showResolved ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                Resolvidos ({notifications.filter(n => n.resolved === true).length})
              </button>
              {showResolved && (
                <div className="space-y-2">
                  {notifications.filter(n => n.resolved === true).map(n => (
                    <div key={n.id} className="p-3 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 opacity-60">
                      <div className="flex items-center gap-3">
                        <CheckCircle size={16} className="text-green-400 shrink-0" />
                        <div className="flex-1">
                          <p className="text-xs font-bold text-slate-500 dark:text-slate-400 line-through">{n.title}</p>
                          <p className="text-[10px] text-slate-400">{n.message}</p>
                          {n.resolvedNote && <p className="text-[10px] text-slate-400 italic">Nota: {n.resolvedNote}</p>}
                          {n.resolvedBy && <p className="text-[10px] text-slate-400">Por: {n.resolvedBy}</p>}
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        {resolveModal && (
          <>
            <div className="fixed inset-0 z-[80] bg-black/60 backdrop-blur-sm" onClick={() => setResolveModal(null)} />
            <div className="fixed left-4 right-4 top-1/2 -translate-y-1/2 z-[90] bg-white dark:bg-[#0a192f] rounded-2xl shadow-xl p-6">
              <div className="flex justify-between items-center mb-4">
                <h4 className="font-bold text-slate-800 dark:text-white">Marcar como Resolvido</h4>
                <button onClick={() => setResolveModal(null)} className="dark:text-white"><X size={18} /></button>
              </div>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">"{resolveModal.title}"</p>
              <input
                className="w-full mb-4 p-2 text-sm border rounded-lg dark:bg-slate-800 dark:border-slate-700 dark:text-white"
                placeholder="Nota (opcional)"
                value={resolveNote}
                onChange={e => setResolveNote(e.target.value)}
              />
              <div className="flex gap-2">
                <button onClick={() => setResolveModal(null)} className="flex-1 py-2 rounded-lg text-sm font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">Cancelar</button>
                <button onClick={() => handleResolveAlert(resolveModal.id, resolveNote || undefined)} className="flex-1 py-2 rounded-lg text-sm font-bold bg-green-500 text-white">Confirmar</button>
              </div>
            </div>
          </>
        )}
